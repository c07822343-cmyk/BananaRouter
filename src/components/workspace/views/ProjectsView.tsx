...
export function ProjectsView() {
  ...
  return (
    <div className="mx-auto max-w-[900px] p-6">
      <h1 className="flex items-center gap-2 text-xl font-medium"><FolderKanban size={20} className="text-[#b45309]" /> Projects</h1>
      ...
      <div className="mt-4 flex gap-2">
        ...
        <button onClick={() => { if (newName.trim()) { createProject(newName.trim()); setNewName(""); } }} className="rounded-full bg-[#b45309] px-4 py-2 text-sm font-medium text-white"><Plus size={14} className="inline" /> Create</button>
      </div>

      <div className="mt-6 grid gap-3 sm:grid-cols-2">
        <button onClick={() => setActiveProject(null)} className={`rounded-2xl border p-4 text-left ${!state.activeProjectId ? "bg-[#FFFBEB] ring-1 ring-[#b45309] dark:bg-[#2a2210]" : "bg-white dark:bg-[#303134]"}`}>
        ...
        {state.projects.map((p) => (
          <div key={p.id} className={`rounded-2xl border p-4 ${state.activeProjectId === p.id ? "bg-[#FFFBEB] ring-1 ring-[#b45309] dark:bg-[#2a2210]" : "bg-white dark:bg-[#303134]"}`}>
